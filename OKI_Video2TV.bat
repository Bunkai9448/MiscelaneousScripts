...
for %%F in ("%INPUT_FOLDER%\*.mkv" "%INPUT_FOLDER%\*.mp4" "%INPUT_FOLDER%\*.avi") do (
    ...
    REM ffmpeg -i "%%F" -vf "scale=720:480:flags=lanczos,subtitles='%INPUT_FOLDER%\\%%~nF.srt'" -codec:v libxvid -b:v 1000k -codec:a libmp3lame -b:a 12k -af aresample=async=1 -f avi "aVideo2TV\tv_%%~nF.avi"
)
